...
export default {
    ...
    mounted: function() {
        ...
        const module_release_id = this.$route.params['moduleReleaseId'];
        ...
        //     }
        // })
        blsc_axios.post(this.SERVER_URL() + "dev/release/list/", body)
        .then(response => {
            console.log("post response dev/release/list/", response);
    ...
    methods: {
        ...
        updateModule(){
            ...
            blsc_axios.post(this.SERVER_URL() + "dev/moduleRelease/", releaseDescription)
            .then(response => {
                //console.log("get /dev/toolbox/", response);
                
                if ( response.status == 200 && response.data.success ) {
                    self.addNotification("Module " + self.moduleRelease.uid + " update succeeded.", "info");
